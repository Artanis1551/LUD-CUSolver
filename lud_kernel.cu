...
void lud_cuda(double *m, int matrix_dim, int cuSolve)
{
  int i=0;
  ...
  if (cuSolve)
  {
    ...
    int bufferSize;
    cusolverStatus_t status = cusolverDnDgetrf_bufferSize(handle, matrix_dim, matrix_dim, m, matrix_dim, &bufferSize);
    
    if (status != CUSOLVER_STATUS_SUCCESS) {
      fprintf(stderr, "cusolverDnDgetrf_bufferSize failed. Status: %d\n", status);
    } else {
      printf("cusolverDnDgetrf_bufferSize succeeded\n");
    }
    
    double* buffer;
    cudaMalloc((void**)&buffer, sizeof(double) * bufferSize);
    fprintf(stderr, "buffer size. Status: %d\n", bufferSize);

    int* devInfo;
    cudaMalloc((void**)&devInfo, sizeof(int));

    stopwatch_start(&func);
    status = cusolverDnDgetrf(handle, matrix_dim, matrix_dim, m, matrix_dim, buffer, devIpiv, devInfo);
    ...
    printf("Time consumed on cusolver(ms): %lf\n", 1000*get_interval_by_sec(&func));
  ...
  else
  {
    ...
    lud_diagonal<<<1,BLOCK_SIZE>>>(m, matrix_dim, i);
    stopwatch_stop(&func);
    printf("Time consumed on the default implementation(ms): %lf\n", 1000*get_interval_by_sec(&func));
  }
}
